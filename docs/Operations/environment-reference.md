...
- `VITE_OPS_API_TOKEN` in `dashboard/.env` matching `OPS_API_TOKEN`
...
`OPS_API_TOKEN`, `VITE_OPS_API_TOKEN`, `TRADING_MODE`, `RISK_PROFILE`

### Other market integrations

- Active now: Polymarket
- Phase 2 scaffold (off by default): Kalshi via `PHASE2_CROSS_VENUE_ENABLED`
...
### RPC infrastructure + Phase 2 (Kalshi)
